...
En las 5 figuras se presentan los tiempos de ejecución de los algoritmos (insertion sort, merge sort, panda sort, quick sort y sort) para distintos tamaños $n$ y tipos de entrada (ascendente, descendente y aleatorio). 
...
Insertion sort destaca con arreglos ascendentes al requerir desplazamientos mínimos, pero sufre significativamente con los descendentes. Merge sort, fiel a su complejidad teórica de $O(n \log n)$, rinde igual en cualquier caso, mostrando solo un levísimo aumento de tiempo en los escenarios aleatorios. 

Panda sort presenta un mal desempeño general propio de una complejidad cercana a $O(n^2)$, ya que no logra sacar provecho de los datos preordenados. Quick sort, al utilizar un pivote central, es más lento en arreglos aleatorios debido a que los arreglos ya ordenados le generan particiones mucho más balanceadas. En resumen, los algoritmos cuadráticos dependen fuertemente del orden original de los datos, mientras que los más eficientes mantienen un rendimiento estable.
...
Se evidencia que quicksort mantiene un consumo de memoria extra prácticamente nulo sin importar el valor de $n$, lo que confirma experimentalmente su naturaleza de ordenamiento in-place. Por el contrario, tanto mergesort como stdsort muestran un crecimiento lineal que se superpone perfectamente, alcanzando aproximadamente los 0.38 MB para arreglos de $10^5$ elementos. 

En el caso de mergesort, este aumento concuerda con la teoría, ya que el algoritmo requiere instanciar arreglos temporales proporcionales a $n$ para poder fusionar las mitades. Por su parte, el consumo observado en la línea de stdsort se explica estrictamente por la implementación evaluada en el código, donde se genera una copia por valor del vector original, elevando así su huella de memoria al mismo nivel que mergesort.